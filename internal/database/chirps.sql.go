...
func (q *Queries) RetrieveChirpsByAuthorID(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, retrieveChirpsByAuthorID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
